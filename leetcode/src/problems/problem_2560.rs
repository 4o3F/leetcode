...
impl Debug for UnionFind {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("UnionFind").field("parent", &self.parent).field("size", &self.size).finish()
    }
}
    ...
    pub fn min_capability(nums: Vec<i32>, k: i32) -> i32 {
        ...
        while current < k {
            ...
            if pair.idx > 0 && visited[pair.idx - 1] {
                ...
                current -= ((uf.size(idx) + 1) / 2) as i32;
...
pub fn run() {
    ...
    tracing::info!("{}", Solution::min_capability_binary_search(vec![2, 3, 5, 9], 2));
    tracing::info!("{}", Solution::min_capability_binary_search(vec![9, 6, 20, 21, 8], 3));
}
