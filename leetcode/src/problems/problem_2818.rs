...
impl Solution {
    ...
    fn generate_left(prim_scores: &Vec<i32>) -> Vec<i32> {
    ...
    fn generate_right(prim_scores: &Vec<i32>) -> Vec<i32> {
        let mut stack = Vec::new();
    ...
    pub fn maximum_score(nums: Vec<i32>, k: i32) -> i32 {
        ...
        for i in 0..nums.len() {
            let i_32 = i as i32;
            if i_32 >= left[i] + 1 && i_32 <= right[i] - 1 {
        ...
        while k > 0 {
            ...
            let pow = range.min(k);
            result *= Self::binpow(num as i64, pow as i64, 1_000_000_007);
